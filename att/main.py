...
from segmentation.model3 import AttU_Net
...
def inference():
    ...
    cwd = os.getcwd()
    output_test_dir = os.path.join('/home/user/dongruixuan /iron/ATTENTION/Att-ce/datasets/segmentation/net_test/test')

    # generate overlapping cropped image for test
    output_test_crop_dir = os.path.join( "/home/user/dongruixuan /iron/ATTENTION/Att-ce/datasets/segmentation/net_test/test_overlap_crop")
    ...
    for item in sorted(os.listdir(os.path.join(output_test_dir, 'images'))):
        tailor(256, 256, os.path.join(output_test_dir, 'images', item), os.path.join(output_test_crop_dir, 'images'),
               region=32)
        tailor(256, 256, os.path.join(output_test_dir, 'labels', item), os.path.join(output_test_crop_dir, 'labels'),
               region=32)
    key_name = 'AttU_Net_model'
