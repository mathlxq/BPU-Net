...
def _segnet(n_classes, encoder, input_height=416, input_width=608,
    ...
    # 补充二分类/通道校验
    assert n_classes >= 2, "类别数需≥2（前景+背景）"
    assert channels in [1, 3], "输入通道仅支持1（灰度）或3（RGB）"
...
if __name__ == '__main__':
    # 测试二分类（晶粒前景+背景）
    m = vgg_segnet(2, channels=1)  # 灰度图设置channels=1
    # m = segnet(2, channels=1)
    # m = mobilenet_segnet(2, channels=1)
    # from keras.utils import plot_model
    # plot_model(m, show_shapes=True, to_file='segnet_binary.png')