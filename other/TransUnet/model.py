...
class Config:
    def __init__(self):
        self.patches = {"size": (16, 16), "grid": None}
...
class ResNetV2(nn.Module):
    def __init__(self, block_units, width_factor):
        ...
        self.root = nn.Sequential(
            Conv2d(3, self.width, kernel_size=7, stride=2, padding=3, bias=False),
        ...
        for i, units in enumerate(block_units):
            block = nn.Sequential(
                *[nn.Conv2d(self.width * (2 ** i), self.width * (2 ** i), kernel_size=3, padding=1)
...
class ViTSegmentation(nn.Module):
    ...
    def forward(self, x):
        ...
        # Transformer编码
        transformer_out, _, features = self.transformer(x)
        decoder_out = self.decoder(transformer_out, features)
        seg_mask = self.segmentation_head(decoder_out)  # (B, num_classes, H, W)
        return seg_mask
...
if __name__ == "__main__":
    ...
    config.n_classes = 2
    model = ViTSegmentation(config, img_size=224, num_classes=2)


    test_input = torch.randn(2, 3, 224, 224)

    with torch.no_grad():
        seg_output = model(test_input)


    print(f"输入尺寸: {test_input.shape}")
    print(f"输出掩码尺寸: {seg_output.shape}")