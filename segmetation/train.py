...
def check(dataset):  # check the output of dataset
    ...
    print(image.size(), " ", mask.size(), ' ', np.unique(mask), " ", last.size(), " ", np.unique(last), " ",
          weight.size(), " ", np.unique(weight))
...
# 新增：动态权重模块单独初始化（保持原有初始化逻辑不变）
def dynamic_weight_init(m):
    if isinstance(m, DynamicWeightModule):
        # 动态权重模块的参数单独初始化
...
class Trainer(object):
    def __init__(self, bs=8, loss_type='vi', multi_layer=True, data_dir=''):
        ...
        print(' batch_size is ', self.batch_size, ' learning_rate is ', self.learning_rate, ' loss_type is ',
              self.loss_type, ' data_dir is ', self.data_dir)
        # 训练集和测试集
        ...
        # 新增：动态权重模块单独初始化
        self.model.dynamic_weight_module.apply(dynamic_weight_init)
        ...
        # 新增：动态权重模块的学习率调整（可选，保持与主模型一致）
        self.criterion = None  # 保留原有字段，避免报错
        self.method = None  # 保留原有字段，避免报错
    ...
    def train(self, epoch, loss_list, plotimgs):
        ...
        for batch_idx, (image, mask, last, weight) in enumerate(self.train_loader):
            ...
            # 新增：动态权重模块的正则化（可选，防止过拟合）
            dynamic_weight_reg = 0.001 * (
                    torch.norm(self.model.module.dynamic_weight_module.alpha) +
                    torch.norm(self.model.module.dynamic_weight_module.beta) +
                    torch.norm(self.model.module.dynamic_weight_module.gamma)
            )
            total_loss = loss + dynamic_weight_reg
            ...
            if (batch_idx + 1) % iterations == 0:
                loss_list.append(float(total_loss.item()))  # 记录总损失
                if plotimgs:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', type=str, default=os.path.join(cwd, "/", "../datasets", "segmentation", "net_train"),
                        help="dataset dir")
